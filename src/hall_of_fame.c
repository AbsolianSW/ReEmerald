...
static void Task_Hof_InitMonData(u8 taskId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES))
        {
            ...
            sHofMonPtr->mon[i].tid = gSaveBlock2Ptr->otData[GetMonData(&gPlayerParty[i], MON_DATA_OT_INDEX)].Id;
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId_Debug(gSaveBlock1Ptr->playerGender, TRUE), TRUE, 120, 72, 6, TAG_NONE);
...
static void HallOfFame_PrintPlayerInfo(u8 unused1, u8 unused2)
{
    ...
    width = GetStringRightAlignXOffset(FONT_NORMAL, gSaveBlock1Ptr->playerName, 0x70);
    AddTextPrinterParameterized3(1, FONT_NORMAL, width, 1, sPlayerInfoTextColors, TEXT_SKIP_DRAW, gSaveBlock1Ptr->playerName);

    trainerId = (gSaveBlock1Ptr->playerTrainerId[0]) | (gSaveBlock1Ptr->playerTrainerId[1] << 8);
